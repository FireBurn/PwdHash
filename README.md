...
reproduces the original PwdHash exactly. Both claims are checked on every change: `node --test
tests/` and `cd Android && ./gradlew testDebugUnitTest` run the extension, the website and the app
...
```bash
node --test tests/                            # extension and website
...
deliberately are documented in [docs/DOMAIN-RULES.md](./docs/DOMAIN-RULES.md).
